...
const createStation = async (req, res) => {
  try {
    ...
    if (!name || !projectId) {
      return res
        .status(400)
        .json({ success: false, message: "name and projectId are required" });
    }
...
const getStationByProjectId = async (req, res) => {
  try {
    ...
    const db = getDB();
    const stations = await stationModel.findStationsByProjectId(db, projectId);
    return res
      .status(200)
      .json({ success: true, count: stations.length, data: stations });
  } catch (err) {
    console.error("getStationByProjectId error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
