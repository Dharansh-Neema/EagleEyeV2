...
const createCamera = async (req, res) => {
  try {
    ...
    if (!name || !inspectionStationId) return res.status(400).json({ success: false, message: 'name and inspectionStationId required' });

    const db = getDB();

    const station = await stationModel.findStationById(db, inspectionStationId);
    if (!station) return res.status(404).json({ success: false, message: 'Inspection station not found' });
    ...
    await db.collection('inspection_stations').updateOne(
      { _id: station._id },
...
const updateCamera = async (req, res) => {
  try {
    ...
    if (!cameraBefore) return res.status(404).json({ success: false, message: 'Camera not found' });
    ...
    if (updateData.name && updateData.name !== cameraBefore.name) {
      await db.collection('inspection_stations').updateOne(
        { _id: cameraBefore.inspection_station_id, 'cameras._id': cameraBefore._id },
...
const getAllCameras = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, count: cameras.length, data: cameras });
  } catch (err) {
...
const getCameraById = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin') {
      ...
      if (!orgIds.includes(camera.organization_id.toString())) return res.status(403).json({ success: false, message: 'Not authorized' });
    }
