...
function validateImage(data) {
  ...
  for (const [field, cfg] of Object.entries(imageSchema)) {
    if (cfg.required && data[field] === undefined) errors[field] = `${field} is required`;
  }
...
async function createImage(db, data) {
  ...
  for (const [field, cfg] of Object.entries(imageSchema)) {
    if (cfg.default !== undefined && data[field] === undefined) {
      data[field] = typeof cfg.default === 'function' ? cfg.default() : cfg.default;
    }
  }
...
async function findImagesForUser(db, orgIds) {
  ...
  return await db.collection('images').find({ 'organization.id': { $in: ids } }).toArray();
}
...
async function deleteImage(db, id) {
  const res = await db.collection('images').deleteOne({ _id: new ObjectId(id) });
